...
export const Game: React.FC<GameProps> = () => {
  ...
  const showComboPopup = useCallback(
    (title: string, subtitle: string | null = null) => {
      ...
      setTimeout(
        () => setComboPopup({ visible: false, title: "", subtitle: null }),
        1200
      );
    },
    []
  ...
  const clearStackablePowerups = useCallback(() => {
    ...
    for (const powerup of stackablePowerupStates) {
      if (
        POWERUP_TYPES[powerup.key]?.stackable &&
        state.activePowerups[powerup.key] === Infinity
      ) {
    ...
    (type: string) => {
      ...
      switch (type) {
        ...
        case "doublePoints":
          ...
          state.activePowerups.doublePoints = powerupInfo.stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "triplePoints":
          ...
          state.activePowerups.triplePoints = powerupInfo.stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "perfectStreak":
          state.perfectStreakActive = true;
          state.activePowerups.perfectStreak = POWERUP_TYPES.perfectStreak
            .stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "comboKeeper":
          ...
          state.activePowerups.comboKeeper = POWERUP_TYPES.comboKeeper.stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "comboBoost":
        ...
        case "magnetize":
          ...
          state.activePowerups.magnetize = POWERUP_TYPES.magnetize.stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "wideGap":
          state.wideGapActive = true;
          state.activePowerups.wideGap = POWERUP_TYPES.wideGap.stackable
            ? Infinity
            : Date.now() + powerupInfo.duration;
          break;
        case "slowRings":
  ...
  const update = useCallback(
    (deltaTime: number) => {
      ...
      if (state.displayScore < state.score) {
        state.displayScore += Math.ceil(
          (state.score - state.displayScore) * 0.2
        );
      ...
      if (state.tinyModeActive) {
        ...
        state.targetSize = Math.min(
          GAME_CONFIG.MAX_PLAYER_SIZE,
          state.targetSize + GAME_CONFIG.PLAYER_GROW_SPEED
        );
      } else {
        state.targetSize = Math.max(
          GAME_CONFIG.MIN_PLAYER_SIZE,
          state.targetSize - GAME_CONFIG.PLAYER_SHRINK_SPEED
        );
      }
      ...
      // Ring spawning
      const now = Date.now();
      const spawnInterval = state.slowTimeActive
        ? state.ringSpawnInterval * 1.5
        : state.ringSpawnInterval;
      ...
      for (const ring of state.rings) {
        ...
        if (ring.isAtPlayer(state.playerSize)) {
          let fitsGap =
            state.ghostActive || ring.playerFitsGap(state.playerSize);
  ...
  return (
    ...
    >
      ...
      <ComboPopup
        ...
        title={comboPopup.title}
        subtitle={comboPopup.subtitle}
      />
