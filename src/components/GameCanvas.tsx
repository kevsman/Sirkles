...
import { Canvas, Circle, Paint, RadialGradient, vec, Group, BlendMode, Path, Skia, Text, useFont } from '@shopify/react-native-skia';
...
export const GameCanvas: React.FC<GameCanvasProps> = ({
    ...
    const playerColor = rainbowActive ? `hsl(${rainbowHue}, 80%, 60%)` : theme.player;
        ...
        <Canvas style={{ width, height }}>
            ...
            <Circle cx={centerX} cy={centerY} r={Math.max(width, height)}>
                <RadialGradient
                    ...
                    colors={[lightenColor(theme.background, 15), theme.background, darkenColor(theme.background, 10)]}
                />
            </Circle>
            ...
            >
                ...
                {Array.from({ length: 20 }).map((_, i) => {
                    ...
                    return <Circle key={`bg-particle-${i}`} cx={x} cy={y} r={size} color={theme.accent} opacity={0.3} />;
                })}

                {/* Background rings */}
                {Array.from({ length: 5 }).map((_, i) => {
                    const r = 100 + i * 120;
                    const alpha = (0.04 + backgroundPulse * 0.02) * (1 - r / Math.max(width, height));
                    return (
                        <Circle
                            key={`bg-ring-${i}`}
                            cx={centerX}
                    ...
                    return (
                        <Group key={`powerup-${index}`}>
                            ...
                            {Array.from({ length: 6 }).map((_, i) => {
                                ...
                                return <Circle key={`sparkle-${i}`} cx={x} cy={y} r={sparkSize} color="#ffffff" />;
                            })}
                        </Group>
                    );
                })}
                ...
                {nearestRing && isPlaying && (
                    <Group>
                        ...
                        />
                        {/* Inner boundary */}
                        <Circle cx={centerX} cy={centerY} r={nearestRing.innerRadius} style="stroke" strokeWidth={2} color="rgba(0, 255, 170, 0.5)" />
                        {/* Outer boundary */}
                        <Circle cx={centerX} cy={centerY} r={nearestRing.outerRadius} style="stroke" strokeWidth={2} color="rgba(0, 255, 170, 0.5)" />
                        {/* Perfect zone */}
                        <Circle
                            cx={centerX}
                            cy={centerY}
                    ...
                    return (
                        <Group key={`ring-${index}`}>
                            ...
                            <Circle
                                ...
                                r={ring.radius}
                                style="stroke"
                                strokeWidth={GAME_CONFIG.RING_THICKNESS + (ring.passed ? 0 : pulse * 4)}
                                color={ring.color}
                                opacity={0.85 + pulse}
                            />
                ...
                {particles.map((p, index) => (
                    <Circle key={`particle-${index}`} cx={p.x} cy={p.y} r={p.size} color={p.color} opacity={p.life} />
                ))}
                ...
                {ghostActive && (
                    <Group>
                        {[3, 2, 1, 0].map((i) => (
                            <Circle key={`ghost-trail-${i}`} cx={centerX} cy={centerY} r={pulsedSize + i * 8} color="rgba(136, 204, 255, 0.1)" />
                        ))}
                    </Group>
                )}
                ...
                {hasShield && (
                    <Group>
                        ...
                        {/* Shield ring */}
                        <Circle cx={centerX} cy={centerY} r={playerSize + 8} style="stroke" strokeWidth={4} color="#ffd700" />
                    </Group>
                )}

                {/* Player glow */}
                <Circle cx={centerX} cy={centerY} r={pulsedSize + 25}>
                    <RadialGradient c={vec(centerX, centerY)} r={pulsedSize + 25} colors={[theme.playerGlow, 'transparent']} />
                </Circle>
                ...
                <Circle cx={centerX} cy={centerY} r={pulsedSize} style="stroke" strokeWidth={2} color="rgba(255, 255, 255, 0.15)" />
                ...
                {powerupProgressPercent > 0 && (
                    <Group>
                        {/* Background ring */}
                        <Circle cx={centerX} cy={centerY} r={playerSize + 35} style="stroke" strokeWidth={3} color="rgba(255, 255, 255, 0.08)" />
